...
import cruds.ejercicios_rutinas, models.ejercicios_rutinas, config.db, schemas.ejercicios_rutinas
...
def read_ejcrtns(skip: int=0, limit: int=10, db: Session=Depends(get_db)):
    db_ejcrtns = cruds.ejercicios_rutinas.get_ejcrnts(db=db,skip=skip, limit=limit)
    return db_ejcrtns
...
def get_ejcrtn_by_ids(ejercicio_id: int, rutina_id: int, db: Session = Depends(get_db)):
    db_ejcrtn = cruds.ejercicios_rutinas.get_ejcrnt_by_ids(db=db, ejercicio_id=ejercicio_id, rutina_id=rutina_id)
...
def create_ejcrtn(ejcrtn: schemas.ejercicios_rutinas.EjcRtnCreate, db: Session=Depends(get_db)):
    db_ejcrtns = cruds.ejercicios_rutinas.get_ejcrnt_by_ids(db, ejercicio_id=ejcrtn.Ejercicio_ID, rutina_id=ejcrtn.Rutina_ID)
    if db_ejcrtns:
        raise HTTPException(status_code=400, detail="Ejercicio - Rutina existente intenta nuevamente")
    return cruds.ejercicios_rutinas.create_ejcrnt(db=db, ejcrtn=ejcrtn)
...
def update_ejcrtn(ejercicio_id: int, rutina_id: int, ejcrtn:schemas.ejercicios_rutinas.EjcRtnUpdate, db: Session = Depends(get_db)):
    db_ejcrtn = cruds.ejercicios_rutinas.update_ejcrtn(db=db, ejercicio_id=ejercicio_id, rutina_id=rutina_id, ejcrtn=ejcrtn)
    if db_ejcrtn is None:
        raise HTTPException(status_code=404, detail="Ejercicio - Rutina not found")
    return db_ejcrtn
...
def delete_ejcrtn(ejercicio_id: int, rutina_id: int, db: Session=Depends(get_db)):
    db_ejcrtns = cruds.ejercicios_rutinas.delete_ejcrt(db=db, ejercicio_id=ejercicio_id, rutina_id=rutina_id )
