...
def create_user(db: Session, user: schemas.users.Usercreate):
    db_user = models.users.User(usuario=user.usuario , password=user.password, created_at=user.crested_at, estatus=user.estatus, Id_persona=user.Id_persona)                          
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def update_user(db: Session, id: int, user: schemas.users.UserUpdate):
    ...
    if db_user:
        for var, value in vars(user).items():
            setattr(db_ user, var, value) if value else None
        db.commit()
        db.refresh(db_user)
    return db_user
